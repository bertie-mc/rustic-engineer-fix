...
- **Do not manually copy a built jar into a player instance or into the packwiz pack.** A
  ...
  The shared `bertie-ci` runner may stage the current build in its ignored, ephemeral
  `.bertie-ci/` test instances; those files must never be committed or treated as pack
  inputs.
